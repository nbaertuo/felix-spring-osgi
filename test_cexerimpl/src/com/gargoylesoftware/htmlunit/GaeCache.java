 ...
 */
public class GaeCache extends com.gargoylesoftware.htmlunit.Cache implements Serializable {
    ...
    private int                      maxSize_         = 400;

    private static javax.cache.Cache entries_;
    static {
        try {
            entries_ = CacheManager.getInstance().getCacheFactory()
                .createCache(Collections.emptyMap());
        } catch (CacheException e) {
            logger.warning("≥ı ºªØcache¥ÌŒÛ" + e.getMessage());
        }
    }
     ...
     */
    public void cacheIfPossible(final WebRequest request, final WebResponse response,
                                final Object toCache) {
    ...
    protected boolean isDynamicContent(final WebResponse response) {
        ...
        final boolean cacheableContent = (expires != null && (expires.getTime() - now > delay) || (expires == null
                                                                                                   && lastModified != null && (now
                                                                                                                               - lastModified
                                                                                                                                   .getTime() > delay)));
    ...
    public Object getCachedObject(final WebRequest request) {
        ...
        final Entry cachedEntry = (Entry) entries_.get(request.getUrl().toString());
        if (cachedEntry == null) {
            return null;
        }
        synchronized (entries_) {
    ...
    public CSSStyleSheet getCachedStyleSheet(final String css) {
        final Entry cachedEntry = (Entry) entries_.get(css);
        if (cachedEntry == null) {
            return null;
        }
        synchronized (entries_) {
