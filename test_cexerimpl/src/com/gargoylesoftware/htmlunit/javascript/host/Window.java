...
public class Window extends SimpleScriptable implements ScriptableWithFallbackGetter, Function {
     ...
     */
    private transient WeakHashMap<Node, ComputedCSSStyleDeclaration> computedStyles_  = new WeakHashMap<Node, ComputedCSSStyleDeclaration>();
    ...
    public boolean jsxFunction_confirm(final String message) {
        ...
        if (handler == null) {
            LOG.warn("window.confirm(\"" + message
    ...
    public WindowProxy jsxFunction_open(final Object url, final Object name, final Object features,
        ...
        if (featuresString != null || replaceCurrentEntryInBrowsingHistory) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("window.open: features and replaceCurrentEntryInBrowsingHistory "
                          + "not implemented: url=[" + urlString + "] windowName=[" + windowName
                          + "] features=[" + featuresString + "] replaceCurrentEntry=["
                          + replaceCurrentEntryInBrowsingHistory + "]");
            }
        }
    ...
    public int jsxFunction_setTimeout(final Object code, int timeout, final Object language) {
        ...
        if (code == null) {
            ...
            final JavaScriptStringJob job = new JavaScriptStringJob(timeout, null, description, w,
                s);
            id = getWebWindow().getJobManager().addJob(job, page);
        } else if (code instanceof Function) {
            final Function f = (Function) code;
            final JavaScriptFunctionJob job = new JavaScriptFunctionJob(timeout, null, description,
                w, f);
            id = getWebWindow().getJobManager().addJob(job, page);
        } else {
    ...
    public void jsxSet_opener(Object newValue) {
        if (getBrowserVersion().hasFeature(BrowserVersionFeatures.GENERATED_177)
            && newValue != opener_) {
    ...
    public boolean jsxFunction_attachEvent(final String type, final Function listener) {
        return getEventListenersContainer().addEventListener(StringUtils.substring(type, 2),
            listener, false);
    }
     ...
     */
    public void jsxFunction_addEventListener(final String type, final Function listener,
    ...
    public void jsxFunction_detachEvent(final String type, final Function listener) {
        getEventListenersContainer().removeEventListener(StringUtils.substring(type, 2), listener,
            false);
    }
     ...
     */
    public void jsxFunction_removeEventListener(final String type, final Function listener,
                                                final boolean useCapture) {
    ...
    */
    ...
    public void triggerOnError(final ScriptException e) {
        ...
        if (o instanceof Function) {
            ...
            final String url = e.getPage().getWebResponse().getWebRequest().getUrl()
                .toExternalForm();
    ...
    private Object getHandlerForJavaScript(final String eventName) {
        ...
        if (handler == null
            && !getBrowserVersion().hasFeature(BrowserVersionFeatures.GENERATED_129)) {
    ...
    public Object getWithFallback(final String name) {
        ...
        if (domNode != null) {
            ...
            if (result instanceof Window) {
            ...
            } else if (result instanceof HTMLUnknownElement
                       && getBrowserVersion().hasFeature(BrowserVersionFeatures.GENERATED_132)) {
                ...
                if (unknownElement.getNodeName().equals("xml")) {
                    ...
                    final Iterator<HtmlElement> children = unknownElement
                        .getHtmlElementDescendants().iterator();
                    if (children.hasNext()) {
    ...
    public void jsxFunction_execScript(final String script, final Object language) {
        ...
        if (language == Undefined.instance || "javascript".equalsIgnoreCase(languageStr)
            || "jscript".equalsIgnoreCase(languageStr)) {
            custom_eval(script);
    ...
    public int jsxFunction_setInterval(final Object code, int timeout, final Object language) {
        ...
        if (code == null) {
            ...
            final String s = (String) code;
            final JavaScriptStringJob job = new JavaScriptStringJob(timeout, timeout, description,
                w, s);
            id = getWebWindow().getJobManager().addJob(job, page);
        } else if (code instanceof Function) {
            final Function f = (Function) code;
            final JavaScriptFunctionJob job = new JavaScriptFunctionJob(timeout, timeout,
                description, w, f);
            id = getWebWindow().getJobManager().addJob(job, page);
        } else {
            throw Context.reportRuntimeError("Unknown type for function.");
     ...
     */
    public ComputedCSSStyleDeclaration jsxFunction_getComputedStyle(final HTMLElement element,
                                                                    final String pseudo) {
     ...
     */
    public Object jsxFunction_showModalDialog(final String url, final Object arguments,
     ...
     */
    public Object jsxFunction_showModelessDialog(final String url, final Object arguments,
                                                 final String features) {
        final WebWindow ww = getWebWindow();
        final WebClient client = ww.getWebClient();
        try {
     ...
     */
    private class DomHtmlAttributeChangeListenerImpl implements DomChangeListener,
                                                    HtmlAttributeChangeListener, Serializable {
        ...
        private void nodeChanged(final DomNode changed) {
            ...
            synchronized (computedStyles_) {
                final Iterator<Map.Entry<Node, ComputedCSSStyleDeclaration>> i = computedStyles_
                    .entrySet().iterator();
                while (i.hasNext()) {
                    final Map.Entry<Node, ComputedCSSStyleDeclaration> entry = i.next();
                    final DomNode node = entry.getKey().getDomNodeOrDie();
                    if (changed == node || changed.getParentNode() == node.getParentNode()
