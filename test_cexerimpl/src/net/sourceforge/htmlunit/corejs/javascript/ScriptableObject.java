...
public abstract class ScriptableObject implements Scriptable, Serializable, DebuggableObject,
     ...
     * 
     */
    private static final long            serialVersionUID              = 8829374243050471600L;
     ...
     *
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#getAttributes(String)
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#setAttributes(String, int)
     */
     ...
     *
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject
     ...
     *
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#getIds()
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#getAttributes(String)
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#setAttributes(String, int)
     */
     ...
     *
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#delete(String)
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#getAttributes(String)
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#setAttributes(String, int)
     */
    ...
    public static final int              CONST                         = PERMANENT | READONLY
                                                                         | UNINITIALIZED_CONST;
    /**
    ...
    private static class Slot implements Serializable {
        ...
        ScriptableObject getPropertyDescriptor(Context cx, Scriptable scope) {
            return buildDataDescriptor(scope, (value == null ? Undefined.instance : value),
                attributes);
        }

    }

    protected static ScriptableObject buildDataDescriptor(Scriptable scope, Object value,
                                                          int attributes) {
     ...
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#has(String, Scriptable)
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#READONLY
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#DONTENUM
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#PERMANENT
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#EMPTY
     */
     ...
     * @see net.sourceforge.htmlunit.corejs.javascript.Function
     * @see net.sourceforge.htmlunit.corejs.javascript.FunctionObject
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject#READONLY
     * @see net.sourceforge.htmlunit.corejs.javascript.ScriptableObject
     ...
     * @since 1.6R2
     */
    public static <T extends Scriptable> String defineClass(Scriptable scope, Class<T> clazz,
    ...
    static <T extends Scriptable> BaseFunction buildClassCtor(Scriptable scope, Class<T> clazz,
                                                              boolean sealed, boolean mapInheritance)
                                                                                                     throws IllegalAccessException,
                                                                                                     InstantiationException,
                                                                                                     InvocationTargetException {
        ...
        if (mapInheritance) {
            ...
            if (ScriptRuntime.ScriptableClass.isAssignableFrom(superClass)
                ...
                String name = ScriptableObject.defineClass(scope, superScriptable, sealed,
                    mapInheritance);
                if (name != null) {
        ...
        for (Method method : methods) {
            ...
            if (annotation instanceof JSGetter || prefix == getterPrefix) {
                if (!(proto instanceof ScriptableObject)) {
                    throw Context.reportRuntimeError2("msg.extend.scriptable", proto.getClass()
                        .toString(), name);
                }
     ...
     * ignoring attributes
     */
    public static void defineProperty(Scriptable destination, String propertyName, Object value,
                                      int attributes) {
     ...
     * @param attributes the attributes of the JavaScript property
     */
    public void defineProperty(String propertyName, Object delegateTo, Method getter,
                               Method setter, int attributes) {
    ...
    private void checkValidPropertyDefinition(Slot slot, ScriptableObject desc) {
        ...
        if (slot == null) { // new property
            ...
            if (isFalse(current.get("configurable", current))) {
                ...
                if (isTrue(current.get("enumerable", current)) != isTrue(getProperty(desc,
                    "enumerable")))
                    throw ScriptRuntime.typeError1("msg.change.enumerable.with.configurable.false",
                        name);
                    ...
                    if (isFalse(current.get("writable", current))) {
                        if (isTrue(getProperty(desc, "writable")))
                            throw ScriptRuntime.typeError1(
                                "msg.change.writable.false.to.true.with.configurable.false", name);

                        if (changes(current.get("value", current), getProperty(desc, "value")))
                            throw ScriptRuntime.typeError1("msg.change.value.with.writable.false",
                                name);
                    }
                } else if (isAccessorDescriptor(desc) && isAccessorDescriptor(current)) {
                    if (changes(current.get("set", current), setter))
                        throw ScriptRuntime.typeError1("msg.change.setter.with.configurable.false",
                            name);

                    if (changes(current.get("get", current), getter))
                        throw ScriptRuntime.typeError1("msg.change.getter.with.configurable.false",
                            name);
                } else {
                    if (isDataDescriptor(current))
                        throw ScriptRuntime.typeError1(
                            "msg.change.property.data.to.accessor.with.configurable.false", name);
                    else
                        throw ScriptRuntime.typeError1(
                            "msg.change.property.accessor.to.data.with.configurable.false", name);
                }
            }
        }
    }
    ...
    protected int applyDescriptorToAttributeBitset(int attributes, ScriptableObject desc) {
        ...
        if (enumerable != NOT_FOUND) {
            attributes = ScriptRuntime.toBoolean(enumerable) ? attributes & ~DONTENUM : attributes
                                                                                        | DONTENUM;
        }

        Object writable = getProperty(desc, "writable");
        if (writable != NOT_FOUND) {
            attributes = ScriptRuntime.toBoolean(writable) ? attributes & ~READONLY : attributes
                                                                                      | READONLY;
        }

        Object configurable = getProperty(desc, "configurable");
        if (configurable != NOT_FOUND) {
            attributes = ScriptRuntime.toBoolean(configurable) ? attributes & ~PERMANENT
            ...
            if (setterObj == null) {
                if (getterSlot.getter != null) {
                    ...
                    if (Context.getContext().hasFeature(
                        Context.FEATURE_HTMLUNIT_WRITE_READONLY_PROPERTIES)) {
    ...
    private Slot getSlot(String name, int index, int accessType) {
        ...
        lastAccessCheck: {
            ...
            if (accessType == SLOT_CONVERT_ACCESSOR_TO_DATA && (slot instanceof GetterSlot))
                break lastAccessCheck;
    ...
    private Slot accessSlot(String name, int index, int accessType) {
        ...
        if (accessType == SLOT_QUERY || accessType == SLOT_MODIFY
            || accessType == SLOT_MODIFY_CONST || accessType == SLOT_MODIFY_GETTER_SETTER
            ...
            synchronized (this) {
                ...
                Slot newSlot = (accessType == SLOT_MODIFY_GETTER_SETTER ? new GetterSlot(name,
                    indexOrHash, 0) : new Slot(name, indexOrHash, 0));
