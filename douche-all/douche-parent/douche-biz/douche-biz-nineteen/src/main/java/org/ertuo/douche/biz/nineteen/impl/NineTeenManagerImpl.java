...
public class NineTeenManagerImpl implements NineTeenManager {
	private final Logger log = org.apache.log4j.Logger
			.getLogger(NineTeenManagerImpl.class);

	private Repository<PostDo> postDao;
	 ...
	 * 
	 * @see
	 * org.ertuo.douche.biz.nineteen.NineTeenManager#getFloorList(java.lang.
	 * String)
	 */
	public Map<String, String> getFloorList(String url) {
		...
		String forumName = url.substring(url.lastIndexOf("/") + 1, url
				.indexOf(".html") - 1)
				+ "thread-";
		// log.info(forumName);
		// 取得楼层
	 ...
	 * 
	 * @see
	 * org.ertuo.douche.biz.nineteen.NineTeenManager#answer(java.lang.String,
	 * java.lang.String)
	 */
	public void answer(String floorId, String newsId) {

		viewId = "http://www.19lou.com/forum-" + floorId + "-thread-" + newsId
				+ "-1-1.html";
		postId = "http://www.19lou.com/post.php?action=reply&fid=" + floorId
				+ "&tid=" + newsId + "&extra=page%3D1";

		if (postDao.get(viewId) != null) {
			// 已经回复过 重复回复
			return;
		}
		...
		try {
			...
			if (page != null) {
				...
				for (HtmlElement htmlElement : textareas) {
					...
					message.setText(DoucheConstant.messages[random
							.nextInt(DoucheConstant.messages.length)]);
				}
				...
				// 优先button方式
				/*
				 * if(!this.buttonSubmit(htmlForm)){ //href方式
	...
	private boolean afterPostOperation(HtmlPage afterPostPage) {
		...
		postDao.persist(new PostDo(viewId, "summersnow8"));
	...
	private void initCategorys() {
		if (categorys.size() == 0) {
			HtmlPage page = webClientLocal
					.getHtmlPageByUrl(NineTeenConstant.host);
			List<HtmlAnchor> anchors = page.getAnchors();
			for (HtmlAnchor htmlAnchor : anchors) {
